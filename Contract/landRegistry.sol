...
contract landRegistration {
    ...
    function addKhatianNew(
        ...
        bytes32 khatianHash = keccak256(
            abi.encodePacked(_khatianiId, _plotHash)
        );
        require(
            khatianMapping[khatianHash].isExist != true,
            "Khatian already exists"
        );
        for (uint j = 0; j < _user.length; j++) {
            require(
                userMapping[_user[j]].isExist == true,
                "User's NID doesn't exist"
            );
        }
    ...
    function addKhatianFromOld(
        ...
        require(
            khatianMapping[_buyFrom].isExist,
            "previous Khatian doesn't exist"
        );
        require(
            khatianMapping[_buyFrom].percentOwn >= _percentOwn,
            "Not enough land to sell"
        );
        for (uint j = 0; j < _user.length; j++) {
            require(
                userMapping[_user[j]].isExist == true,
    ...
    function getKhatianHasById(
        uint id
    ) public view returns (bytes32 khatianHash) {
        return khatianArray[id];
    }

    function getKhatianByHash(
        bytes32 khatianHash
    )
        public
        view
        returns (
            ...
            uint16 percentOwn,
            bytes32 buyFrom,
            bytes32[] memory sellTo,
            uint16[] memory sellPercentage,
            uint[] memory ownerArray,
            uint16[] memory perOwnerPercentage
        )
    {
        require(
            khatianMapping[khatianHash].isExist == true,
            "Khatian doesn't exist"
        );
    ...
    function addPlot(
        ...
        string memory _thana,
        uint16 _JLNo,
        uint16 _plotNo,
        uint8 _plotType,
        uint32 _plotArea,
        string memory _plotMap
    ) public {
        ...
        bytes32 plotHash = keccak256(
            abi.encodePacked(_division, _district, _thana, _JLNo, _plotNo)
        );
        ...
        plotMapping[plotHash].plotMap = _plotMap;
        ...
        )
    {
        ...
        plotMap = plotMapping[plotHash].plotMap;
    }

    function creatNewUser(
        string memory _fullName,
        string memory _fatheName,
        string memory _district,
        string memory _thana,
        uint24 _postCode,
    ...
    function getUserOwnedByUid(
        uint _uid
    ) public view returns (uint number, bytes32[] memory khatianList) {
    ...
    function toUpper(string memory str) internal pure returns (string memory) {
        ...
        for (uint i = 0; i < bStr.length; i++) {
            uint8 charValue = uint8(bStr[i]);
            if ((charValue >= 97) && (charValue <= 122)) {
                bUpper[i] = bytes1(charValue - 32);
            } else {
                bUpper[i] = bStr[i];
            }
        }
